...
void ATLAS_LABEL(Double_t x,Double_t y,Color_t color, Double_t tsize) 
{
  ...
  l.DrawLatex(x,y,"#it{ATLAS} #bf{Internal}");
}
...
void myText(Double_t x,Double_t y,Color_t color, TString text, Double_t size, bool NDC = 1) {

  //Double_t tsize=size; // 0.05;
  TLatex l; //l.SetTextAlign(12); 
  l.SetTextSize(size); 
  if (NDC) l.SetNDC();
  l.SetTextColor(color);
